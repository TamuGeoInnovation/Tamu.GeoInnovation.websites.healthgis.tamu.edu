...
var Lightbox = {
	...
	showBoxString : function(content, boxWidth, boxHeight, dontShowClose){
		this.setLightboxDimensions(boxWidth, boxHeight);
		this.lightboxType = 'string';
		var contentsString = $('boxContentsString');
		contentsString.innerHTML = content;
		
		var contentsImage = $('boxContentsImage');
		contentsImage.innerHTML = "";
		
		this.showBox(dontShowClose);
		return false;
	},

	showBoxImage : function(href) {
		this.lightboxType = 'image';
		var contents = $('boxContentsImage');
	...
	showBoxImageAndString : function(href, content, boxWidth, boxHeight, dontShowClose) {
		this.lightboxType = 'imageAndString';
		this.setLightboxDimensions(boxWidth, boxHeight);
		var contentsString = $('boxContentsString');
		contentsString.innerHTML = content;
		if ($('lightboxImage'))
		{
		    Element.remove($('lightboxImage'));
		}
		var contentsImage = $('boxContentsImage');
		var objImage = document.createElement("img");
		objImage.setAttribute('id','lightboxImage');
		contentsImage.appendChild(objImage);
		imgPreload = new Image();
		imgPreload.onload=function(){
			objImage.src = href;
			Lightbox.showBox(dontShowClose);
		}
		imgPreload.src = href;
		return false;
	},

	showBoxByID: function(id, boxWidth, boxHeight, dontShowClose)
	{
	...
	showBox : function(dontShowClose) 
	{
	    if (dontShowClose)
	    {
	        $('close').hide();
	...
	{				  
		...
		lightboxtext += '<div style="float:left" id="boxContentsImage"></div>';
		lightboxtext += '<div style="float:left; overflow:auto;" id="boxContentsString"></div>';
		lightboxtext += '</div>';
		var body = document.getElementsByTagName("body").item(0);
